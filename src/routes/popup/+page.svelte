<script>
  ...
  let template =
    "https://public.tableau.com/workbooks/SuperstoreDashboard_16384653251620.twb";
  ...
  function modifyCSS(cssCode) {
    ...
    const urlContentPattern =
      /(?:background(?:-image)?|content)\s*:\s*url\((.*?)\)/g;
    ...
    newCSSCode = newCSSCode.replace(cssBlockPattern, (match, cssBlock) => {
      if (cssBlock.includes("--modified:")) {
        ...
        if (hasTransform) {
          ...
          const modifiedTransform = cssBlock.replace(
            transformPattern,
            (match, p1) => {
  ...
  function extractClassInfo(cssCode) {
    ...
    classAndRulesList.forEach(
      ({ originalClass: originalClassName, newClass: className, css }) => {
        const { title, description, modified } =
          extractTitleAndDescription(css);
        classInfoList.push({
          originalClass: originalClassName,
          newClass: className,
          title,
